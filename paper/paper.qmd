...
From @fig-runrate-numofwickets, both the distribution of current run rates of teams when a wicket occurs and when a wicket does not occur are approximately normally distributed around 8.5. The variability for both distributions also appears to be the same. However, the distribution of run rate for balls that are not wickets occur has more outliers than for wickets occurrences.
